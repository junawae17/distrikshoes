		...
		<table class="table table-bordered">
			<tbody>
				...
				<tr>
					<th>Total Stok</th>
					<td>
					<?php $total_stock = $this->db->query(" select * FROM datastock__ LEFT JOIN msize__ on msize__.id_size = datastock__.id_size where product_id = ".$rows['product_id']." and status_stock = 1 ")->result(); 
						$total = count($total_stock);

						echo $total;
					?>
					<!-- <?=$rows['stock']?> -->

					<button class="btn btn-xs btn-success btn-sm btn-outline faa-parent animated-hover" data-toggle="modal" data-target=".modalstock" data-placement="top" title="" data-loading-text="Loading..."><i class="fa fa-plus faa-vertical"></i> Stock</button>
				...
				<tr>
					...
					<?php $images = explode(',',$rows['images_product']); $noId=0;
						...
						<?php $noId++; } }?>
						<button class="btn btn-xs btn-success btn-sm btn-outline faa-parent animated-hover" data-toggle="modal" data-target=".modalgambar" data-placement="top" title="" data-loading-text="Loading..."><i class="fa fa-plus faa-vertical"></i> Gambar</button>
					</td>
				</tr>
				<tr>
					<th>Informasi</th>
...
<div class="modal fade modalgambar" tabindex="-1" role="dialog" style="display: none;">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
			...
			<div class="modal-body">
			 <form action="<?=base_url()?>Admin/savegambarProduct" class="form-horizontal" method="post" enctype="multipart/form-data" >
			 	<input type="hidden" name="product_id" value="<?=$rows['product_id']?>">
				<input type="hidden" name="images_old" value="<?=$rows['images_product']?>">
				<input type="hidden" name="action" value="update">
				<div class="row form-group">
...
<div class="modal fade modalstock" tabindex="-1" role="dialog" style="display: none;">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
			...
			<div class="modal-body">
			 <form action="<?=base_url()?>Admin/savestock" class="form-horizontal" method="post" enctype="multipart/form-data" >
				 	<input type="hidden" name="product_id" value="<?=$rows['product_id']?>">
					<input type="hidden" name="action" value="update">
				<div class="row form-group">
					<table class="table">
						<?php
							$total_stock = $this->db->query(" select * FROM datastock__ LEFT JOIN msize__ on msize__.id_size = datastock__.id_size where product_id = ".$rows['product_id']." and status_stock = 1 ")->result();
							$data_stock = NULL;
						...
						<tr class="odd gradeX">
							...
							<td><a href="<?=base_url().'Admin/hapusstock/'.$value->id_stock.'/'.$rows['product_id'] ?>" onclick="return confirm('Anda yakin ingin hapus Stock ini');" class="btn btn-danger faa-parent animated-hover"><span class="fa fa-trash-o faa-vertical"></span> Hapus</a></td>
						</tr>
